...
The project structure follows best practices for reproducible research with compiled extensions:

```text
├── R/                  # Helper functions for DAG logic and visualization
├── src/                # Compiled simulation code (Rust/C++)
├── _targets.R          # Pipeline definition (Make-like workflow)
├── analysis/           # Quarto notebooks with step-by-step narrative
│   ├── 01-dag-design.qmd       # Defining the business logic
│   ├── 02-simulation.qmd       # The "Ground Truth" generator
│   └── 03-causal-estimation.qmd # The model comparison
├── data/               # Simulated datasets (generated by the pipeline)
└── .rv                 # Environment configuration
```
