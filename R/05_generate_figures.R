...
# ------------------------------------------------------------------------------
cat("Step 1/9: Generating Causal DAG and adjustment set...\n")
...
# ------------------------------------------------------------------------------
cat("Step 2/9: Simulating 50,000 customers (true effect baked in)...\n")
...
# ------------------------------------------------------------------------------
cat("Step 3/9: Building the Simpson's Paradox figure...\n")
...
# ------------------------------------------------------------------------------
cat("Step 4/9: Fitting naive models (omit confounder, keep collider)...\n")
...
# ------------------------------------------------------------------------------
cat("Step 5/9: Fitting causal models (DAG-guided adjustment)...\n")
...
# ------------------------------------------------------------------------------
cat("Step 6/9: Validating estimates against ground truth (marginal ATE)...\n")
...
# ------------------------------------------------------------------------------
# 7b. Predictive performance (test-set AUC) -- reported NEUTRALLY
# ------------------------------------------------------------------------------
cat("Step 7/9: Predictive performance (test-set AUC) across specifications...\n")
auc_tbl <- evaluate_predictive_performance(df)
cat("\nTest-set ROC AUC (predictive accuracy is a DIFFERENT goal than causal validity):\n")
cat(strrep("-", 80), "\n")
print(auc_tbl, n = Inf)
cat(strrep("-", 80), "\n")
cat("Note: AUC measures prediction only. The kitchen-sink model may predict well\n")
cat("      yet still get the CAUSAL estimate wrong. Do not equate the two.\n\n")

# ------------------------------------------------------------------------------
# 8. Bootstrap CI for the causal ATE
# ------------------------------------------------------------------------------
cat("Step 8/9: Bootstrap 95% CI for the causal ATE...\n")
...
# ------------------------------------------------------------------------------
cat("Step 9/9: E-value sensitivity analysis (robustness to latent impatience)...\n")
...
cat("  Test-set AUC by spec   :\n")
for (i in seq_len(nrow(auc_tbl))) {
  cat(sprintf("    - %-32s %.3f\n", auc_tbl$Specification[i], auc_tbl$Test_AUC[i]))
}
cat(strrep("=", 80), "\n")
